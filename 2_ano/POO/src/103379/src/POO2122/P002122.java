...
public class P002122 {
	...
	private static void alinea2(PrintStream out) {
		out.println("\nAlínea 2) ----------------------------------\n");
		EventManager em = new EventManager("Fun & Leisure"); // modificar
		
		// Adicione a seguir o código necessário para a leitura do ficheiro 
		ArrayList<String> linhas = new ArrayList<>();
		try {
			Scanner file = new Scanner(new FileReader("./src/POO2122/events.txt"));
			while(file.hasNext()){
				linhas.add(file.nextLine());
			}
			for (String s : linhas) {
				String[] div_info = s.split(",");
				if(s.charAt(0) == '#'){
					Client aux_cl = em.addClient(div_info[0], div_info[1]);
					em.addEvent(aux_cl, LocalDate.parse(div_info[2]));
				}else{
