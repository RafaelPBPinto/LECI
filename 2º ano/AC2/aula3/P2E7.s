...
while:	lw	$s3,LATE($s0)		# READ		
	...
	lw	$s3,PORTB($s0)		# READ
	andi	$s3,$s3,0x0004		# isola bit2 
	srl	$s5,$s3,2		# 
	sw	$s3,PORTB($s0)		# WRITE
	
if:	beqz	$s5,else		# if(RB2 != 0)
...
else:	andi	$s4,$s2,0x0001		# isola bit menos significativo
	sll	$s4,$s4,3		# passa para bit mais significativo
	xori	$s4,$s4,0x0008		# negar bit
	srl	$s2,$s2,1		#
	or	$s2,$s2,$s4		#
